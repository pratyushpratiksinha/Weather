...
extension CDCity {
    func convertToCityTVCModel() -> CityTVCModel {
        return CityTVCModel(id: Int(self.id), index: Int(self.index), cityName: self.cityName ?? "" , countryName: self.countryName ?? "", weatherDescription: self.weatherDescription ?? "", backgroundImage: self.backgroundImage, temperatureCurrent: self.temperatureCurrent, temperatureHigh: self.temperatureHigh, temperatureLow: self.temperatureLow, location: CLLocation(latitude: self.latitude, longitude: self.longitude), scale: self.scale ?? "", forecast: convertToCityForecastWeatherDataCVCModel(), condition: convertToCityConditionWeatherDataCVCModel())
    }
